...
const { calculateAutoAdjustedPrice } = require('./pricing');
...
async function syncCurrentStock(shop_id, item_ids) {
  ...
  for (const item_id of uniqueIds) {
    ...
    const item = await dbGet(`SELECT category, is_active FROM item_master WHERE item_id = ?`, [item_id]);
    if (item) {
      const cat = (item.category || '').toUpperCase();
      if (cat === 'RECAP' || cat === 'USED TIRE') {
        const stockSum = await dbGet(`SELECT COALESCE(SUM(current_quantity), 0) AS total_qty FROM current_stock WHERE item_id = ?`, [item_id]);
        const totalQty = stockSum ? stockSum.total_qty : 0;
        if (totalQty <= 0 && item.is_active !== 0) {
          await dbRun(`UPDATE item_master SET is_active = 0 WHERE item_id = ?`, [item_id]);
        } else if (totalQty > 0 && item.is_active === 0) {
          await dbRun(`UPDATE item_master SET is_active = 1 WHERE item_id = ?`, [item_id]);
        }
      }
    }
  }
...
function findOrCreateDotVariant(parent_item_id, dot_number, unit_cost, selling_price, created_by) {
  ...
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM item_master WHERE item_id = ?`, [parent_item_id], (err, item) => {
      ...
      const trueParentId = item.dot_number ? (item.parent_item_id || parent_item_id) : parent_item_id;

      db.get(
        `SELECT * FROM item_master 
         WHERE category = ? AND COALESCE(brand,'') = ? AND COALESCE(design,'') = ? AND COALESCE(size,'') = ? 
           AND dot_number = ? AND is_active = 1`,
        [item.category, item.brand || '', item.design || '', item.size || '', dotLabel],
        (err2, existingChild) => {
          ...
          if (existingChild) {
            ...
            const newPrice = selling_price != null
              ...
              : (Math.abs(costDelta) > 0.001 ? calculateAutoAdjustedPrice(existingChild.selling_price, existingChild.unit_cost, newCost, existingChild.category) : existingChild.selling_price);
            ...
            db.get(`SELECT * FROM item_master WHERE item_id = ?`, [trueParentId], (err3, parent) => {
              ...
              const newPrice = selling_price != null
                ? parseFloat(selling_price)
                : (Math.abs(costDelta) > 0.001 ? calculateAutoAdjustedPrice(parent.selling_price, parent.unit_cost, newCost, parent.category) : parent.selling_price);
