...
async function getDailySummary(shop_id, date) {
  try {
    ...
    // Capture sales-only cash before applying manual entries
    ...
    manualEntries.forEach(e => {
      ...
      // CASH and GCASH manual ledger entries both affect the physical cash-on-hand pool
      if (e.entry_type.startsWith('CASH')) {
        breakdown.CASH += val;
        if (isOut) manualCashOut += e.amount; else manualCashIn += e.amount;
      } else if (e.entry_type.startsWith('GCASH')) {
        breakdown.CASH += val;
        ...
        if (desc.includes('BPI')) breakdown.BPI += val;
        else breakdown.BDO += val;
      } else {
        breakdown.CASH += val;
        if (isOut) manualCashOut += e.amount; else manualCashIn += e.amount;
      }
