...
async function getDailySummary(shop_id, date) {
  try {
    ...
    const purchaseData = await dbGet(
      `SELECT 
        (SELECT COALESCE(SUM(total_amount), 0) FROM purchase_header WHERE shop_id = ? AND is_void = 0 AND DATE(purchase_date) = ?)
        as manualPurchases,
        (SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE shop_id = ? AND status = 'RECEIVED' AND DATE(received_at) = ? AND payment_mode != 'CASH')
        as orderPurchases`,
    ...
    // Add receivable collections (exclude opening balance down payment rows)
    ...
    const payablePayments = await dbAll(
      `SELECT amount, payment_method FROM payable_payments 
       WHERE shop_id = ? AND is_void = 0 AND DATE(payment_date) = ?`,
      [shop_id, date]
    );
    ...
    // Total purchases for KPI (Manual + Orders)
    const totalPurchasesKpi = (purchaseData.manualPurchases || 0) + (purchaseData.orderPurchases || 0);

    // Deduct only manual purchases from cash pool (Orders are handled via payable_payments)
