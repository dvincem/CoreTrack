...
router.get("/labor-log/:shop_id", (req, res) => {
  ...
  const business_date = date || getLocalTodayYYYYMMDD();
  db.all(
...
router.post("/labor-log", (req, res) => {
  ...
  const bdate = business_date || getLocalTodayYYYYMMDD();
  db.run(
    ...
    (err, rows) => {
      ...
      for (const r of rows) {
        if (!map[r.staff_id]) {
          map[r.staff_id] = { 
            staff_id: r.staff_id, 
            full_name: r.full_name || r.staff_id, 
            staff_code: r.staff_code, 
            bale_deducted: r.bale_deducted || 0,
            services: [], 
            commissions: [] 
          };
        }
